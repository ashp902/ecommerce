...
def product_page(request, id):
    ...
    inventory_item = InventoryItem.objects.only('count', 'price', 'discount').filter(product_id=id)[0]
...
def place_order(request):
    ...
    print(products)
    items = [(InventoryItem.objects.filter(product_id=product['product_id'])[0], product) for product in products]
